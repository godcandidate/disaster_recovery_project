...
# IAM Role for CloudWatch to invoke API Gateway
resource "aws_iam_role" "cloudwatch_api_gateway" {
  name = "dr-cloudwatch-api-gateway-role-${var.environment}"
...
# IAM Policy for CloudWatch to invoke API Gateway
resource "aws_iam_policy" "cloudwatch_api_gateway" {
  name        = "dr-cloudwatch-api-gateway-policy-${var.environment}"
  description = "Policy for CloudWatch to invoke API Gateway"
...
resource "aws_iam_role_policy_attachment" "cloudwatch_api_gateway" {
  role       = aws_iam_role.cloudwatch_api_gateway.name
  policy_arn = aws_iam_policy.cloudwatch_api_gateway.arn
}

# CloudWatch Alarm Action to invoke API Gateway
resource "aws_cloudwatch_metric_alarm" "dr_failover_action" {
  alarm_name          = "dr-failover-action-${var.environment}"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  metric_name         = "FailoverRequired"
  namespace           = "DR/Custom"
  period              = 60
  statistic           = "Maximum"
  threshold           = 0
  alarm_description   = "This alarm is triggered by the composite alarm to invoke the API Gateway"
  
  alarm_actions = [var.api_gateway_invoke_url]
  
  tags = merge(
    {
      Name        = "dr-failover-action-${var.environment}"
      Environment = var.environment
    },
    var.tags
  )
